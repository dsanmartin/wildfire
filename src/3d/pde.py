...
def grad_pressure(p: np.ndarray, params: dict) -> np.ndarray:
    ...
    # Compute grad(p)
    grad_p = compute_gradient(p, dx, dy, dz, (False, False, True))
    return grad_p
...
def Phi(t: float, C: np.ndarray, params: dict) -> np.ndarray:
    ...
    Y_ = -Y_f * K(T) * H(T) * Y 
    # Boundary conditions
    u_, v_, w_, T_, Y_ = boundary_conditions(u_, v_, w_, T_, Y_, params)
    return np.array([u_, v_, w_, T_, Y_])

def boundary_conditions(u: np.ndarray, v: np.ndarray, w: np.ndarray, T: np.ndarray, Y: np.ndarray, params: dict) -> np.ndarray:
    """
    ...
    u_dn, v_dn, w_dn, T_dn, Y_dn = params['values_dead_nodes']
...
def solve_tn(t_n: float, y_n: np.ndarray, dt: float, method: callable, params: dict) -> tuple[np.ndarray, np.ndarray]:
    ...
    t_n : float
        Current time.
    y_n : numpy.ndarray (5, Ny-1, Nx-1, Nz)
    ...
    Returns
    -------
    y_np1 : np.ndarray (5, Ny-1, Nx-1, Nz-1)
        Array with the solution at the next time step.
    p : np.ndarray (Ny-1, Nx-1, N_z)
    ...
    Ut, Vt, Wt = y_np1[:3].copy()
    # Solve Pressure problem
    p = solve_pressure(Ut, Vt, Wt, params)
    grad_p = grad_pressure(p, params)
    # Velocity correction (Chorin's projection method)
    y_np1[:3] = y_np1[:3] - dt / rho * grad_p
    Ut, Vt, Wt, Tt, Yt = y_np1.copy()
    # Update boundary conditions
    y_np1 = boundary_conditions(Ut, Vt, Wt, Tt, Yt, params)
    if bound:
        # Bound mass fraction
        y_np1[4, y_np1[3] < Y_min] = Y_min 
        y_np1[4, y_np1[3] > Y_max] = Y_max 
        # Bound temperature
        y_np1[3, y_np1[2] < T_min] = T_min
        y_np1[3, y_np1[2] > T_max] = T_max
...
def data_post_processing(z: np.ndarray, p: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    Parameters
    ----------
    z : numpy.ndarray (Nt, 5, Ny-1, Nx-1, Nz)
        Variables u, v, w, T, Y, without periodic BC.
    p : numpy.ndarray (Nt, Ny-1, Nx-1, Nz)
        Pressure without periodic BC.

    Returns
    -------
    tuple of numpy.ndarray
        Tuple containing the post-processed data arrays u, v, w, T, Y, and p.
...
def solve_pde(z_0: np.ndarray, params: dict) -> tuple[np.ndarray, np.ndarray]:
    ...
    if NT == 1: # Save all time steps
        # Approximation
        z = np.zeros((Nt, z_0.shape[0], Ny - 1, Nx - 1, Nz)) 
        ...
        # Approximation
        z = np.zeros((Nt // NT + 1, z_0.shape[0], Ny - 1, Nx - 1, Nz)) 
