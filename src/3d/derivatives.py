...
def compute_first_derivative(phi: np.ndarray, h: float, axis: int, periodic: bool = True, type: str = 'central') -> np.ndarray:
    """
    Compute the first derivative of a 3D scalar field along a given axis.
    ...
    derivative in the general case. If the boundary is not periodic, the boundary
    nodes are computed using a first-order forward/backward or central difference scheme.
    ...
    if type == 'forward':
        ...
        if periodic == False:
            ...
            elif axis == 2: # Fix boundary in z - O(dz)
                phi_h[:,:,-1] = (phi[:,:,-1] - phi[:,:,-2]) / h
    elif type == 'backward':
        phi_h = (phi - phi_im1) / h
        if periodic == False:
...
def compute_first_derivative_half_step(phi: np.ndarray, h: float, axis: int, periodic: bool = True) -> np.ndarray:
    """
    Computes the derivative of a 3D array `phi` along a given `axis` using a central difference scheme.
    ...
    phi : numpy.ndarray (Ny, Nx)
        The 3D array to compute the derivative of.
    h : float
        The grid spacing.
    axis : int
...
def compute_second_derivative(phi: np.ndarray, h: float, axis: int, periodic: bool = True) -> np.ndarray:
    """
    Compute the second derivative of a 3D scalar field along a given axis.
...
def compute_gradient(phi: np.ndarray, dx: float, dy: float, dz: float, periodic_axes: tuple) -> np.ndarray:
    """
...
def compute_laplacian(phi: np.ndarray, dx: float, dy: float, dz: float, periodic_axes: tuple) -> np.ndarray:
    """
...
def compute_curl(vphi: np.ndarray, dx: float, dy: float, dz: float, periodic_axes: tuple) -> np.ndarray:
    """
    ...
    dphiz_y = compute_first_derivative(vphiz, dy, axis=0, periodic=periodic_axes[0]) # dphiz/dy
    ...
    dphiz_x = compute_first_derivative(vphiz, dx, axis=1, periodic=periodic_axes[1]) # dphiz/dx
...
def compute_divergence(vphi: np.ndarray, dx: float, dy: float, dz: float, periodic_axes: tuple) -> np.ndarray:
    """
...
def compute_first_derivative_plots(phi: np.ndarray, h: float, axis: int) -> np.ndarray:
    ...
    if axis == 3: # Fix boundary in y - O(dy^2)
        phi_h[:,:,:, 0] = (-3 * phi[:,:,:, 0] + 4 * phi[:,:,:, 1] - phi[:,:,:, 2]) / (2 * h) # Forward
        phi_h[:,:,:,-1] = (3 * phi[:,:,:, -1] - 4 * phi[:,:,:, -2] + phi[:,:,:, -3]) / (2 * h) # Backward
    return phi_h

def compute_gradient_plots(phi: np.ndarray, dx: float, dy: float, dz: float) -> np.ndarray:
    """
    ...
    dphi_z = compute_first_derivative_plots(phi, dz, axis=3) # dphi/dz
    gradient = np.array([dphi_x, dphi_y, dphi_z])
    return gradient

def compute_curl_plots(vphi: np.ndarray, dx: float, dy: float, dz: float) -> np.ndarray:
    """
    ...
    """
    vphix, vphiy, vphiz = vphi
    dphiz_y = compute_first_derivative_plots(vphiz, dy, axis=1)# dphiz/dy
    dphiy_z = compute_first_derivative_plots(vphiy, dz, axis=3) # dphiy/dz
    dphix_z = compute_first_derivative_plots(vphix, dz, axis=3) # dphix/dz
    dphiz_x = compute_first_derivative_plots(vphiz, dx, axis=2) # dphiz/dx
...
def compute_divergence_plots(vphi: np.ndarray, dx: float, dy: float, dz: float) -> np.ndarray:
    """
    ...
    dphiz_z = compute_first_derivative_plots(vphiz, dz, axis=3) # dphiz/dz
    divergence = dphix_x + dphiy_y + dphiz_z
    return divergence