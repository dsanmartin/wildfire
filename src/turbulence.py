...
def turbulence_2D_test(U: tuple[np.ndarray, np.ndarray] , T: np.ndarray, args: dict) -> np.ndarray:
    ...
    # Intermediary terms
    # with damping
    sgs_x_damp = 2 * mod_S * fw * rho_v * (fwx * ux + 0.5 * fwy * (vx + uy)) #+ mod_S * fw ** 2 * (rho_x)
    sgs_y_damp = 2 * mod_S * fw * rho_v * (0.5 * fwx * (uy + vx) + fwy * vy) #+
    sgs_x_damp += mod_S * fw ** 2 * (rhox * ux + 0.5 * rhoy * (vx + uy))
    sgs_y_damp += mod_S * fw ** 2 * (0.5 * rhox * (uy + vx) + rhoy * vy)
    # without damping
    ...
    # SGS stresses
    sgs_x = -2 * l ** 2 * (sgs_x_no_damp * fw ** 2 * rho_v + sgs_x_damp) # x-component of SGS stresses
    sgs_y = -2 * l ** 2 * (sgs_y_no_damp * fw ** 2 * rho_v + sgs_y_damp) # y-component of SGS stresses
    # SGS thermal energy
    sgs_T_no_damp = 1 / (2 * mod_S) * (psi_x * Tx  + psi_y * Ty) + mod_S * (Txx + Tyy) # No damping terms
    sgs_T_damp = 2 * fw * mod_S * rho_v * (fwx * Tx + fwy * Ty) # Damping terms
    sgs_T_damp += mod_S * fw ** 2 * (rhox * Tx + rhoy * Ty)
    sgs_T = -l ** 2 * c_p / Pr * (sgs_T_no_damp * fw ** 2 * rho_v + sgs_T_damp) # SGS thermal energy * c_p
