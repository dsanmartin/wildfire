...
t_min, t_max = 0, 5 # Time interval [t_min, t_max] in s
...
# Numerical grid
Nx, Ny, Nt = 256, 256, 2001 # Number of nodes per axis
...
k = 2.07e-5 # Thermal diffusivity in m^2 s^{-1} (Air: 2.07e-5) # k = kk / (rho * C_p)
...
A = 1e9 # Pre-exponential factor in s^{-1}. 1e9 According to Asensio 2002
...
h_rad = 0*1e-7#5.67e-8 # Stefan-Boltzmann constant in W m^{-2} K^{-4} or kg s^{-3} K^{-4}
Y_thr = 1 # Threshold to add solid fuel force
Y_f = 1 # Extra parameter to control the fuel consumption rate
T_hot = T_pc + 150 #T_inf + 600 #Temperature of fire in K
...
# Temperature 
# Shape
shape_type = 'half gaussian' # 'plate' or 'half gaussian'
# Location
