...
# y_min, y_max = 0, 100 # Distance interval [y_min, y_max] in m
#t_min, t_max = 0, 10 # Time interval [t_min, t_max] in s

# Numerical grid
Nx, Ny, Nt = 512, 512, 60001 # Number of nodes per axis
# Nx, Ny, Nt = 512, 512, 20001 # Number of nodes per axis
...
k = 1e-1 # Thermal diffusivity in m^2 s^{-1} # 1e1

# Fluid equations parameters
nu = 1e0 # Viscosity in m^2 s^{-1} 
...
Pr = nu / k # Pr = 1e1 # Prandtl number 1. / .71 air
...
H_R = 200 # Heat energy per unit of mass (wood) in J kg^{-1}. About 21.20e6 for wood according to https://en.wikipedia.org/wiki/Heat_of_combustion
...
h = 5e-2 # Convection coefficient in W m^{-2} K^{-1}
Y_thr = .5 # Threshold to add solid fuel force
Y_f = 0.025 # Extra parameter to control the fuel consumption
